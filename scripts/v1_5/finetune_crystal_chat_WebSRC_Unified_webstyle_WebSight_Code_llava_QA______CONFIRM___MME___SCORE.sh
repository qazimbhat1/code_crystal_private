...
#SBATCH --output=finetune_crystal_chat_tokenizer_correct_Websight_websrc_code_QA_llava_______CONFIRM____MME___SCORE.txt
#SBATCH --nodes=4
...
deepspeed --hostfile /lustre/scratch/shared-folders/vision-project/Code/qazim.bhat/LLaVA/playground/data/llava_finetune_data/hostfile_new.txt llava/train/train_mem.py \
    ...
    --bf16 True \
    --output_dir ./checkpoints/llava-v1.5-finetune-crystal_chat-tokenizer-correct_Websight_websrc_code_QA_llava_______CONFIRM____MME___SCORE \
    --num_train_epochs 1 \
